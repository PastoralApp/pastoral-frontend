...
@Component({
  selector: 'app-coordenacao-evento-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './coordenacao-evento-form.component.html',
  styleUrl: './coordenacao-evento-form.component.scss'
})
export class CoordenacaoEventoFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      if (this.isCoordGrupo() && this.userGrupoId()) {
    ...
    if (this.isEditMode() && this.eventoId()) {
      ...
      this.eventoService.update(this.eventoId()!, updateData).subscribe({
        next: () => {
          ...
          this.router.navigate(['/coord-grupo/eventos']);
        },
        error: (err) => {
